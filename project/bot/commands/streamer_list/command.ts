...
export const options: CommandOptions = {
    name: "list",
    description: "Will list all monitored streamers.",
    category: "streamer",
    ignoreBots: true,
    args: [],
    ...
    execute: async (bot, context) => {
        ...
        if (memberValue) {
            ...
            if (Object.keys(memberJSON).length != 0) {
                ...
                embed.setDescription(`The following streamers are being monitored:\n\`\`\`${Object.keys(memberJSON).join(", ")}\`\`\``);
        ...
        embed.setDescription("There are no monitored streamers!");
